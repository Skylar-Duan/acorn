...
const RE = {
  ...
  repWeekly: /每个?(?:周|星期)的?(?:周|星期)?([一二三四五六日天]+)/g,
  ...
  repMonthEnd: /每个?月的?月?(?:最后一天|末|底)/g,
  repMonthly: new RegExp(`每个?月的?(${NUM})[号日]`, "g"),
  // 光写「每周」「每个月」、没说哪天:按这件事写了的日期推(没写就按今天)。
  // 否定前瞻把带了下文的留给上面那几条——「每周末」「每周一」「每月15号」「每月底」不能被它截走;
  // 「每月初」「每月中」这次不认,也不许被截成「每月」+「初」,整串原样留给标题;
  // 「每月的…」「每周的…」「每周2次」「每周两次」同理——前面那几条没认出来的,整串留给标题,不许被截成光写的循环
  repWeekBare: /每个?(?:周|星期)(?![一二三四五六日天末的\d两次])/g,
  repMonthBare: new RegExp(`每个?月(?!${NUM}|[末底初中的]|最后)`, "g"),
  // 天数跟自定义面板同一个范围(1–365),所以阿拉伯数字放到三位,回调里再卡上限
  repEveryN: new RegExp(`每(\\d{1,3}|${NUM})天`, "g"),
  repDaily: /每天/g,
...
export function parseSubtaskInput(
  ...
  // 只扫「每」「每月」:没打 ~ 的「每个月」是正文,一个字不动(有 ~ 领着的「~每个月」早被循环那条整串吃掉了)
  const title = tidyTitle(r.title, keepNewlines, (l) =>
    l.replace(/(?:^|\s)每月?(?=\s|$)/g, " ").replace(/\s+/g, " ").trim(),
  );
  return { ...r, title };
}
