...
// 更新等下一次开机也不迟。
// 「版本过旧」那一屏在 v1.9.1 拆了，现在是弹一次的「已有更新版橡果」框（NewerDataDialog），
...
import { displayVersion, shortVersion } from "../core/version";
    ...
    <div className="overlay">
      <div className="modal update-modal">
        <div className="update-head">
          <h2 className="serif">有新版本 {displayVersion(info.version)}</h2>
          <p>
            这台设备上是 {shortVersion()}
        ...
        <p className="update-hint update-note">
 ...
 *
 * 现在的去处：NewerDataDialog 的「现在更新」。数据是新版本写的，那个框劝人升级，
 * 就得当场给一条升级的路——不然等于一边说「升级后就能看到」、一边让人自己去找。
 * （v1.9.1 之前它挂在「版本过旧」那一整屏墙上，那屏把设置页整个挡住了，
 * 现在墙拆了，应用照常渲染，那个框只弹一次、取消了照常用，但就近给个按钮仍然比让人绕一圈强。）
