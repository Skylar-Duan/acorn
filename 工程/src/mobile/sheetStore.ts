...
export type SheetKind =
   ...
   *  登录着的时候管头像、名字、自动登录和一键退出；没登录的时候整张纸就一颗登录按钮。
   *  为什么给它一张纸而不是直接跳设置：用户说的是「主页面。右上角。方便一键登入登出」——
   *  换个账号本来是两下的事，走设置页要先跳页、再展开一节、再往下找 */
  | { kind: "account" }
  /** 顺延到哪天（09-21 统一口径）：左滑「顺延」、动作单「顺延」、今天页「全部顺延」共用这一张。
   *  存的是「哪几行」的 id 而不是行本身：纸开着的时候别处改了数据，以点选项那一刻为准。
   *  subId 有值 = 这一行是那条子任务 */
  | { kind: "postpone"; rows: { taskId: string; subId?: string }[] }
