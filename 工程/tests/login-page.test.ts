...
describe("设置页「账号」未登录时只剩一个入口", () => {
  ...
  it("右上角头像那条更近的路，两处说的是同一件事（v1.15.0 手机，v1.15.1 起两端都有）", () => {
    // 右上角那颗头像点开（手机是账号纸，桌面是小面板）也有一颗「退出登录」，接的是这一页的
    // 「只退出登录，保留本机」。**清空本机那条路只在这一页有**——它得先当场同步成功才放行，
    // 不该出现在一点就中的地方。两处各写各的话，早晚有一处让人以为按了会清数据。
    // v1.15.1 起这句两端都显示，所以不再点「手机上」这个端名，也不再包在 isMobile 里
    expect(accountPanelSource).toContain("右上角那颗头像点开也能退出登录");
    expect(accountPanelSource).not.toContain("手机上「今天」右上角");
    expect(accountPanelSource).not.toContain("{isMobile && (");
