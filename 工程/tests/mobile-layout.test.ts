...
describe("P1-4 / P1-5 给手机用户看的东西，别指他做不到的操作", () => {
  ...
  // v1.14.1 这句话跟着「一句话记事」并进「通用」时改了措辞（并进去之后叫「快捷用语指南」），
  // 但要钉的还是同一件事：安卓开不了独立窗口，别在手机上许一个不存在的窗口
  it("设置页「打开用法」那句按平台分叉（安卓开不了独立窗口）", () => {
    const s = nl(settingsSource);
    expect(s).toContain('? "点「打开用法」，会开一个单独的窗口，里面是一组可以照着抄的例子"');
