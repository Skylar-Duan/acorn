...
- **视图**：随手记 · 今天（逾期置顶）· 习惯 · **计划** · **已完成** · 日历 · 专注 · 统计 · 用法 · 回收站（侧栏「更多」里常驻，写着每条还剩几天，可单条恢复 / 彻底删除，任务拖过去即删）
...
- **子任务链可折叠**：嫌一件事占太多行，点行首的小三角收起来——只留「下一步」那一条，行尾标 +N 说明后面还有几条；「计划」右上角有个总开关一次收起全部。做完的子任务在任务卡里自动沉到最下面
...
- **不会用？**「随手记」右边有个 `?`，侧栏「更多 → 用法」是一页举例卡片：左边你会打的那句，右边橡果当场解析出的结果
...
- **云账号与跨设备同步**（可选，不登录一切照旧）：邮箱注册 + 验证码激活，登录后多台设备同一份任务。本机那份和云端那份是**合起来**的不是覆盖，同一件事两边都改过以改得晚的为准；断网、服务器挂了都不影响本地使用
...
- 子任务链折叠、任务卡里做完的子任务沉底
- 任务卡的「整句改」
- 用法速查页（`views/Guide.tsx`，样式里已经写好手机版：一列大卡、按下有反馈）
...
├─ views/           今天 · 计划（列表 × 四象限）· 已完成 · 通用列表 · 习惯 · 日历 · 专注 · 统计 · 用法 · 设置
