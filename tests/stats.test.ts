...
describe("byList", () => {
  ...
  it("收件箱第一行 id=null，清单按 order 升序，全零清单也出现", () => {
    const r = byList(tasks, LISTS, WS, WE);
    expect(r.map((x) => x.name)).toEqual(["收件箱", "工作", "生活"]);
  ...
  it("done 只看 doneAt 落区间；open 不看区间；已删除两边都不算", () => {
    ...
    expect(inbox).toEqual({ id: null, name: "收件箱", done: 0, open: 1 });
  });

  it("空任务时每个清单加收件箱都在且全零", () => {
...
describe("weeklyReview", () => {
  ...
  it("completed 排列与 perList 分组顺序一致（供导出切片还原）", () => {
    ...
    const nameOf = (id: string | null) =>
      id === null ? "收件箱" : (LISTS.find((l) => l.id === id)?.name ?? "收件箱");
